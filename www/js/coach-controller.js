angular.module('coachController', ['starter.services', 'checklist-model', 'ui.calendar', 'ngCordova'])
      ...
      MyServices.searchAthlete({
        ...
        if (ci == j) {
          _.each(data.data.results, function (key) {
            temp.push(key.athlete._id);
            // $scope.athletes.push(key.athlete);
          })
          var diff = _.difference(temp, $scope.tempBlogAthlete);

          _.each(data.data.results, function (n, i) {
            // console.log("n.athlete._id", n.athlete._id);
            // console.log("diff[i]", diff[i]);
            _.each(diff, function (m) {
              if (n.athlete._id == m) {
                $scope.athletes.push(n.athlete)
              }
            });
          })
    ...
    if ($stateParams.id) {
      MyServices.getOneBlog({
        ...
        if (response.data) {
          ...
          _.each($scope.formData.athleteArray, function (key) {
            // console.log("key.athlete from backend", key.athlete);
            $scope.tempBlogAthlete.push(key._id);
          });
        } else {
          $scope.formData = {};
        }
  ...
  .controller('CoachTrainingDiaryCtrl', function ($scope, $ionicModal, $ionicLoading, uiCalendarConfig, MyServices, $ionicScrollDelegate, $timeout, $filter, $location) {
    ...
    $scope.changeAthlete = function (value) {
      $scope.athleteData = [];
      $scope.showLoading('Loading...', 10000);
      MyServices.getAthletePlans(value, function (data) {
        if (data.value === true) {
          $scope.hideLoading();
          console.log(data);
          ...
          $scope.hideLoading();
          $scope.showPlan = false;
          $scope.showLoading('No Training Plan assigned', 2000);
          console.log(data);
        }
      });
    };
  ...
  .controller('CoachAthletesCoachingDetailCtrl', function ($scope, $ionicModal, $stateParams, MyServices, $state, $ionicPopup) {
    ...
    $scope.rejectRequest = function () {
      ...
      MyServices.Unsubscribeathlete($scope.unsubscribe, function (response) {
        $state.go('app.coach-athletes-coaching');
      });
    };
